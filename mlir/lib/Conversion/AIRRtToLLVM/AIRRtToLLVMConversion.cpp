//===- ConvertLaunchFuncToAIRRtRuntimeCalls.cpp - MLIR GPU lowering passes --===//
//
...
#include "mlir/Conversion/AIRRtToLLVM/AIRRtToLLVMPass.h"
...
static constexpr const char *kAIRRtBinaryStorageSuffix = "_airbin_cst";

namespace {

class AIRRtToLLVMConversionPass
    : public AIRRtToLLVMConversionPassBase<AIRRtToLLVMConversionPass> {
public:
  AIRRtToLLVMConversionPass() = default;

  AIRRtToLLVMConversionPass(bool kernelBarePtrCallConv)
      : AIRRtToLLVMConversionPass() {
    //if (this->kernelBarePtrCallConv.getNumOccurrences() == 0)
    //  this->kernelBarePtrCallConv = kernelBarePtrCallConv;
  }

  AIRRtToLLVMConversionPass(const AIRRtToLLVMConversionPass &other)
      : AIRRtToLLVMConversionPassBase(other) {}

  // Run the dialect converter on the module.
  void runOnOperation();

private:
  Option<std::string> airrtBinaryAnnotation{
      *this, "airrt-binary-annotation",
      llvm::cl::desc("Annotation attribute string for AIR binary"),
      llvm::cl::init("airrt.binary")};
  //llvm::cl::Option<bool> kernelBarePtrCallConv
...
class ConvertOpToAIRRtRuntimeCallPattern : public ConvertOpToLLVMPattern<OpTy> {
public:
  explicit ConvertOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter)
...
protected:
  ...
  FunctionCallBuilder streamCreateCallBuilder = {
      "martStreamCreate", llvmPointerType /* void *stream */, {}};
  FunctionCallBuilder streamDestroyCallBuilder = {
      "martStreamDestroy", llvmVoidType, {llvmPointerType /* void *stream */}};
  FunctionCallBuilder streamSynchronizeCallBuilder = {
      "martStreamSynchronize",
      llvmVoidType,
  ...
  FunctionCallBuilder eventCreateCallBuilder = {
      "martEventCreate", llvmPointerType /* void *event */, {}};
  FunctionCallBuilder eventDestroyCallBuilder = {
      "martEventDestroy", llvmVoidType, {llvmPointerType /* void *event */}};
  FunctionCallBuilder eventSynchronizeCallBuilder = {
      "martEventSynchronize",
      llvmVoidType,
  ...
  FunctionCallBuilder setDefaultDeviceCallBuilder = {
      "martSetDefaultDevice",
      llvmVoidType,
      {llvmInt32Type /* uint32_t devIndex */}};
};


/// A rewrite pattern to convert gpu.alloc operations into an AIRRt runtime
/// call. Currently it supports ROCm (HIP).
class ConvertAllocOpToAIRRtRuntimeCallPattern
    : public ConvertOpToAIRRtRuntimeCallPattern<airrt::AllocOp> {
public:
  ConvertAllocOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter)
      : ConvertOpToAIRRtRuntimeCallPattern<airrt::AllocOp>(typeConverter) {}

private:
  LogicalResult
  matchAndRewrite(airrt::AllocOp allocOp, OpAdaptor adaptor,
...
/// call. Currently it supports CUDA and ROCm (HIP).
class ConvertWaitOpToAIRRtRuntimeCallPattern
    : public ConvertOpToAIRRtRuntimeCallPattern<airrt::WaitAllOp> {
public:
  ConvertWaitOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter)
      : ConvertOpToAIRRtRuntimeCallPattern<airrt::WaitAllOp>(typeConverter) {}

private:
  LogicalResult
  matchAndRewrite(airrt::WaitAllOp waitOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const;
};

/// A rewrite pattern to convert gpu.wait async operations into a GPU runtime
/// call. Currently it supports CUDA and ROCm (HIP).
class ConvertWaitAsyncOpToAIRRtRuntimeCallPattern
    : public ConvertOpToAIRRtRuntimeCallPattern<airrt::WaitAllOp> {
public:
  ConvertWaitAsyncOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter)
      : ConvertOpToAIRRtRuntimeCallPattern<airrt::WaitAllOp>(typeConverter) {}
...
class ConvertLaunchFuncOpToAIRRtRuntimeCallPattern
    : public ConvertOpToAIRRtRuntimeCallPattern<airrt::LaunchFuncOp> {
public:
  ConvertLaunchFuncOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter,
                                             StringRef airrtBinaryAnnotation,
                                             bool kernelBarePtrCallConv)
      : ConvertOpToAIRRtRuntimeCallPattern<airrt::LaunchFuncOp>(typeConverter),
        airrtBinaryAnnotation(airrtBinaryAnnotation),
        kernelBarePtrCallConv(kernelBarePtrCallConv) {}

private:
  Value generateParamsArray(airrt::LaunchFuncOp launchOp, OpAdaptor adaptor,
                            OpBuilder &builder) const;
  Value generateKernelNameConstant(StringRef moduleName, StringRef name,
                                   Location loc, OpBuilder &builder) const;

  LogicalResult
  matchAndRewrite(airrt::LaunchFuncOp launchOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override;

  llvm::SmallString<32> airrtBinaryAnnotation;
  bool kernelBarePtrCallConv;
};
//RESTART ERASING FROM HERE
class EraseAIRRtModuleOpPattern : public OpRewritePattern<ModuleOp> {
  using OpRewritePattern<ModuleOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(ModuleOp op,
                                PatternRewriter &rewriter) const override {
    
    if (!op->hasAttr("airrt.kernel"))
      return rewriter.notifyMatchFailure(
          op, "The op doesn't have the kernel attr.");
    // GPU kernel modules are no longer necessary since we have a global
    // constant with the CUBIN, or HSACO data.
    //int numRegions = 0;
    //int numBl = 0;
    ////for (auto &reg : op->getRegions()) {
    //  //for (auto &bl : reg.getBlocks())
    ////auto reg = op->getBodyRegion();
    //auto &reg = op.getBodyRegion();
    //for (auto &bl : reg.getBlocks())
    //  numBl++;
    ...
    //  numRegions++;
    //}
    //llvm::errs() << "The size of the module regions is " << numRegions << " numBl=" << numBl << "\n";
...
/// call. Currently it supports CUDA and ROCm (HIP).
class ConvertMemcpyOpToAIRRtRuntimeCallPattern
    : public ConvertOpToAIRRtRuntimeCallPattern<airrt::DmaMemcpyNdOpSrcDst> {
public:
  ConvertMemcpyOpToAIRRtRuntimeCallPattern(LLVMTypeConverter &typeConverter)
      : ConvertOpToAIRRtRuntimeCallPattern<airrt::DmaMemcpyNdOpSrcDst>(typeConverter) {}

private:
  LogicalResult
  matchAndRewrite(airrt::DmaMemcpyNdOpSrcDst memcpyOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override;
};
...
void AIRRtToLLVMConversionPass::runOnOperation() {
  LLVMTypeConverter converter(&getContext());
  RewritePatternSet patterns(&getContext());
  LLVMConversionTarget target(getContext());

  //allowing just partial lowering until it's done
  ...
  populateAsyncStructuralTypeConversionsAndLegality(converter, patterns,
                                                    target);
  populateAIRRtToLLVMConversionPatterns(converter, patterns, airrtBinaryAnnotation,
                                      /*kernelBarePtrCallConv=*/false); //ugly substitution
...
LogicalResult ConvertAllocOpToAIRRtRuntimeCallPattern::matchAndRewrite(
    airrt::AllocOp allocOp, OpAdaptor adaptor,
    ConversionPatternRewriter &rewriter) const {
...
static bool isAIRRtAsyncTokenType(Value value) {
  return value.getType().isa<airrt::AsyncTokenType>();
}
...
////LogicalResult ConvertAsyncYieldToAIRRtRuntimeCallPattern::matchAndRewrite(
////    async::YieldOp yieldOp, OpAdaptor adaptor,
////    ConversionPatternRewriter &rewriter) const {
////  if (llvm::none_of(yieldOp.operands(), isAIRRtAsyncTokenType))
////    return rewriter.notifyMatchFailure(yieldOp, "no airrt async token operand");
////
...
////    if (!isAIRRtAsyncTokenType(operand.get()))
////      continue;
...
LogicalResult ConvertWaitOpToAIRRtRuntimeCallPattern::matchAndRewrite(
    airrt::WaitAllOp waitOp, OpAdaptor adaptor,
    ConversionPatternRewriter &rewriter) const {
...
// property.
LogicalResult ConvertWaitAsyncOpToAIRRtRuntimeCallPattern::matchAndRewrite(
    airrt::WaitAllOp waitOp, OpAdaptor adaptor,
    ConversionPatternRewriter &rewriter) const {
  if (!waitOp.asyncToken())
...
// return %array
Value ConvertLaunchFuncOpToAIRRtRuntimeCallPattern::generateParamsArray(
    airrt::LaunchFuncOp launchOp, OpAdaptor adaptor, OpBuilder &builder) const {
...
// }
Value ConvertLaunchFuncOpToAIRRtRuntimeCallPattern::generateKernelNameConstant(
...
LogicalResult ConvertLaunchFuncOpToAIRRtRuntimeCallPattern::matchAndRewrite(
    airrt::LaunchFuncOp launchOp, OpAdaptor adaptor,
    ConversionPatternRewriter &rewriter) const {
  ...
  auto kernelModule = SymbolTable::lookupNearestSymbolFrom<ModuleOp>(
      launchOp, launchOp.getKernelModuleName());

  llvm::errs() << "The symbol you're looking for here is " << launchOp.getKernelModuleName() << "\n";
  assert(kernelModule && "expected a kernel module");

  auto binaryAttr =
      kernelModule->getAttrOfType<StringAttr>(airrtBinaryAnnotation);
  if (!binaryAttr) {
    kernelModule.emitOpError()
        << "missing " << airrtBinaryAnnotation << " attribute";
    return failure();
  }

  //SmallString<128> nameBuffer(kernelModule.getName());
  SmallString<128> nameBuffer(launchOp.getKernelModuleName());

  //nameBuffer(llvm::StringRef(kernelModule.str().c_str())
  //SmallString<128> nameBuffer;
  //if(kernelModule.getName().hasValue())
  //  SmallString<128> nameBuffer(kernelModule.getName().getValue());

  nameBuffer.append(kAIRRtBinaryStorageSuffix);
  llvm::errs() << "In the case of AIRRt, this is the bin name " << nameBuffer.str() << "\n";
  Value data =
  ...
  auto zero_64bits = rewriter.create<LLVM::ConstantOp>(loc, llvmInt64Type,
  ...
  Value dynamicSharedMemorySize = zero;
  ...
  launchKernelCallBuilder.create(
      loc, rewriter,
      {function.getResult(0), zero_64bits, zero_64bits,
       zero_64bits, zero_64bits, zero_64bits,
       zero_64bits, dynamicSharedMemorySize, stream, kernelParams,
       /*extra=*/nullpointer});
...
LogicalResult ConvertMemcpyOpToAIRRtRuntimeCallPattern::matchAndRewrite(
    airrt::DmaMemcpyNdOpSrcDst memcpyOp, OpAdaptor adaptor,
    ConversionPatternRewriter &rewriter) const {
...
mlir::createAIRRtToLLVMConversionPass(bool kernelBarePtrCallConv) {
  return std::make_unique<AIRRtToLLVMConversionPass>(kernelBarePtrCallConv);
}

void mlir::populateAIRRtToLLVMConversionPatterns(LLVMTypeConverter &converter,
                                               RewritePatternSet &patterns,
                                               StringRef airrtBinaryAnnotation,
                                               bool kernelBarePtrCallConv) {
  converter.addConversion(
      [context = &converter.getContext()](airrt::AsyncTokenType type) -> Type {
        return LLVM::LLVMPointerType::get(IntegerType::get(context, 8));
      });

  patterns.add<ConvertAllocOpToAIRRtRuntimeCallPattern,
               //ConvertDeallocOpToAIRRtRuntimeCallPattern,
               //ConvertHostRegisterOpToAIRRtRuntimeCallPattern,
               ConvertMemcpyOpToAIRRtRuntimeCallPattern,
               //ConvertMemsetOpToAIRRtRuntimeCallPattern,
               //ConvertSetDefaultDeviceOpToAIRRtRuntimeCallPattern,
               ConvertWaitAsyncOpToAIRRtRuntimeCallPattern,
               ConvertWaitOpToAIRRtRuntimeCallPattern>(converter);
               //ConvertAsyncYieldToAIRRtRuntimeCallPattern>(converter);
  ...
  //             ConvertSetDefaultDeviceOpToAIRRtRuntimeCallPattern,
  //             ConvertWaitAsyncOpToAIRRtRuntimeCallPattern,
  //             ConvertWaitOpToAIRRtRuntimeCallPattern,
  //             ConvertAsyncYieldToAIRRtRuntimeCallPattern>(converter);
  patterns.add<ConvertLaunchFuncOpToAIRRtRuntimeCallPattern>(
      converter, airrtBinaryAnnotation, kernelBarePtrCallConv);
  //erasing the module brings an error related to having to have 1 module
  patterns.add<EraseAIRRtModuleOpPattern>(&converter.getContext());
}
