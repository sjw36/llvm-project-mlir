...
GlobalSyncRewritePattern::matchAndRewrite(GlobalSyncOp op,
  ...
  auto semaphoreTy = MemRefType::get({0}, semaElemTy);
  ...
  if (!func->hasAttr(gs_attr_name)) {
    ...
    gsync_cnt = func->getAttrOfType<IntegerAttr>(gs_attr_name).getInt();
  }
  func->setAttr(gs_attr_name, b.getI32IntegerAttr(++gsync_cnt));
  ...
  // > }
  {
    auto ifop = b.create<scf::IfOp>(loc, thdCond, false);
  ...
  b.create<amdgpu::LDSBarrierOp>(loc);
...
void RockLowerGlobalSyncPass::runOnOperation() {
  ...
  target.addLegalDialect<arith::ArithDialect, memref::MemRefDialect,
			 ...
			 amdgpu::AMDGPUDialect, rock::RockDialect>();
